...
func (repo *MyPostgresRepo) InsterNewRestraurant(ctx context.Context, restaurant *models.Restaurant) error {
	...
	_, err := repo.db.QueryContext(ctx, "INSERT INTO restaurants (id, name, city, owner, address, description, days_open, capacity, specialties) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9);", restaurant.Id, restaurant.Name, restaurant.City, restaurant.Owner, restaurant.Address, restaurant.Description, DaysOpen, Capacity, Specialties)
	return err
}

func (repo *MyPostgresRepo) GetAllRestaurants(ctx context.Context, offset int) ([]models.Restaurant, error) {
	rows, err := repo.db.QueryContext(ctx, "SELECT id, name, city, owner, address, description, address, description, days_open, capacity, specialties FROM restaurants ORDER BY created_at DESC LIMIT 20 OFFSET $1;", offset)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		err = rows.Scan(&res_id, &res_name, &res_city, &res_owner, &res_address, &res_des, &res_days, &res_cap, &res_spec)
		if err != nil {
			return nil, err
		}
...
func (repo *MyPostgresRepo) GetRestaurantByName(ctx context.Context, name string, offset int) ([]models.Restaurant, error) {
	rows, err := repo.db.QueryContext(ctx, "SELECT id, name, city, owner, address, description, days_open, capacity, specialties FROM restaurants WHERE name = $1 ORDER BY created_at DESC LIMIT 20 OFFSET $2;", strings.ToLower(name), offset)
	if err != nil {
		return nil, err
	}
...
func (repo *MyPostgresRepo) GetRestaurantByCity(ctx context.Context, city string, offset int) ([]models.Restaurant, error) {
	rows, err := repo.db.QueryContext(ctx, "SELECT id, name, city, owner, address, description, days_open, capacity, specialties FROM restaurants WHERE city = $1 ORDER BY created_at DESC LIMIT 20 OFFSET $2;", strings.ToLower(city), offset)
	if err != nil {
		return nil, err
	}
...
func (repo *MyPostgresRepo) GetMyRestaurants(ctx context.Context, id string) ([]models.Restaurant, error) {
	rows, err := repo.db.QueryContext(ctx, "SELECT id, name, city, owner, address, description, days_open, capacity, specialties FROM restaurants WHERE owner = $1 ORDER BY created_at DESC;", id)
	if err != nil {
		return nil, err
	}
...
func (repo *MyPostgresRepo) UpdateRestaurant(ctx context.Context, restaurant_data *models.Restaurant, user_id string) error {
	...
	_, err := repo.db.ExecContext(ctx, "UPDATE restaurants SET name=$1, description=$2, days_open=$3, capacity=$4, specialties=$5 WHERE id=$6 AND owner=$7;", restaurant_data.Name, restaurant_data.Address, DaysOpen, Capacity, Specialties, restaurant_data.Id, user_id)
	return err
}
